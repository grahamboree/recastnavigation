...
void CrowdTool::handleMenu()
{
	...
	if (ImGui::CollapsingHeader("Options", 0, true, true))
	{
		ImGui::Indent();
		bool agentParamsChanged = false;
		agentParamsChanged |= ImGui::Checkbox("Optimize Visibility", &params->m_optimizeVis);
		agentParamsChanged |= ImGui::Checkbox("Optimize Topology", &params->m_optimizeTopo);
		agentParamsChanged |= ImGui::Checkbox("Anticipate Turns", &params->m_anticipateTurns);
		agentParamsChanged |= ImGui::Checkbox("Obstacle Avoidance", &params->m_obstacleAvoidance);
		agentParamsChanged |= ImGui::SliderFloat("Avoidance Quality", &params->m_obstacleAvoidanceType, 0.0f, 3.0f);
		agentParamsChanged |= ImGui::Checkbox("Separation", &params->m_separation);
		agentParamsChanged |= ImGui::SliderFloat("Separation Weight", &params->m_separationWeight, 0.0f, 20.0f);

		if (agentParamsChanged)
		{
			// Round to whole values.
			params->m_obstacleAvoidanceType = floor(params->m_obstacleAvoidanceType + 0.5f);
			m_state->updateAgentParams();
		}
		ImGui::Unindent();
	}

	if (ImGui::CollapsingHeader("Selected Debug Draw", 0, true, true))
	{
		ImGui::Indent();
